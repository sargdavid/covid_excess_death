...
dt2 <- dt1[, .(Date = as.Date(paste0(dt1$Month_Code,
               ...
               Cause = factor(ICD.10.113.Cause.List,
                              ...
                              labels = gsub(x = unique(ICD.10.113.Cause.List),
                                            pattern = "#",
                                            replacement = "")),
               Population = as.integer(dt1$Population),
               Deaths = as.integer(dt1$Deaths),
               daysmonth = dt1$daysmonth,
...
tmp[, pop2010 := Population[Date == "2010-01-01"],
    by = State]
tmp[, pop2010pct := 100*Population/pop2010]
```
...
p2 <- ggplot(tmp,
  ...
  scale_x_continuous(breaks = seq.Date(from = min(tmp$Date),
                                       to = max(tmp$Date),
                                       by = 365.25)) +
  ggtitle("Population (% January 2010)") +
  theme_bw() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90,
                                   hjust = 1))
