...
```{r}
spectrum(diab_ca$Rate[diab_ca$set == "trn"])
...
In this case, 1/12 frequency seems to be the only one. Extend this to the training and validation sets, and calculate MAPE.

```{r}
diab_ca$freq1 <- NA
diab_ca$freq1[diab_ca$set == "trn"] <- m11$seasonal + m11$trend
...
**ToDo:**  
1. How to extend the predictions from Fourier analysis to the testing and validation sets?  
2. Calculate MAPE for all 3 sets.  
3. How to make this analysis for all 20 states in a single model (similar to random effects model idea)?  
4. repeat this for each cause of death.  
