...
## State populations
```{r}
dt0 <- data.table(read.csv("data/JOINT_HistRec_till23_state_month_rev7_Tableau.csv"))

dt0 <- unique(dt0[, c("State",
                      "Year",
                      "Population")])
```
...
## New data
```{r}
dt1 <- data.table(read.csv("data/Cause_State_week_deaths.csv"))
```

## Remove multiple year aggregates
```{r}
unique(dt1$Year)
# NONE?
```
...
dt2 <- dt1[, .(Year = Year,
               ...
               Cause = factor(dt1$Cause.Subgroup),
               Deaths = as.integer(Number.of.Deaths),
               Death_avg = as.integer(dt1$Average.Number.of.Deaths.in.Time.Period))]
               # daysmonth = daysmonth,
...
## Death vs. Average Death
```{r, fig.width=5,fig.height=5}
plot(dt2$Death_avg ~ dt2$Deaths,
     pch = ".")
```
